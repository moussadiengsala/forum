...
func (s *Session) InitSession(db *sql.DB, user model.User, w http.ResponseWriter) error {
...
func (s *Session) CreateSession(w http.ResponseWriter, r *http.Request, response *model.Reponse, db *model.DB, usr model.User) {
	if response.StatusCode != http.StatusOK {
		// No need to create a session if the response status code is not OK
		return
	}

	sessionErr := s.InitSession(db.Instance, usr, w)
	if sessionErr != nil {
